# AGENTS.md — Notes for AI agents (gregMod.CableTracer)

Repo: https://github.com/mleem97/gregMod.CableTracer · License: Apache-2.0 · Version: see `VERSION` (1.1.0).

MelonMod for Data Center. Cable tracing/highlighting.
...
4. **Verify changes:** before reporting done, build the mod (`dotnet build gregMod.CableTracer.csproj -c Release` or `./build.sh CableTracer` from `ModRepositories/`).
...
- Deploy only with `./build.sh CableTracer --deploy`.

## Hard rules

- Tracing is read-only highlighting — never mutate cable or port state.
...
- `src/CableTracerMod.cs` — MelonMod entry, prefs, toggle.
- `src/TracerManager.cs` — trace state and highlighting.
- `src/CableTracerConfig.cs` — pref data. `src/MyPluginInfo.cs` — metadata.
